...
# Defensive: also re-apply patch to the local tarball's source dir (in case it was modified).
# The tarball under /opt/openfoam-v2512/tarballs/kokkos-kernels-5.1.0-plan-i.tar.gz must
# stay valid; verify Plan I marks present.
PLANI_TGZ=/opt/openfoam-v2512/tarballs/kokkos-kernels-5.1.0-plan-i.tar.gz
if [[ -f "$PLANI_TGZ" ]]; then
  PLANI_MARKS=$(tar xzOf "$PLANI_TGZ" kokkos-kernels-5.1.0/cmake/kokkoskernels_components.cmake 2>/dev/null | grep -c "Plan I disabled")
